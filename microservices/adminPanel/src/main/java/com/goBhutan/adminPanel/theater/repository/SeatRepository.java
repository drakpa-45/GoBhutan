...
public interface SeatRepository extends JpaRepository<Seat, String> {
    ...
    @Query("SELECT COUNT(s) FROM BusSeat s WHERE s.hall.id = :hallId AND s.isActive = true")
    Long countByHallIdAndIsActive(@Param("hallId") String hallId);

    @Query("SELECT COUNT(s) FROM BusSeat s WHERE s.hall.id = :hallId AND s.isBlocked = false AND s.isActive = true")
    Long countAvailableSeatsByHallId(@Param("hallId") String hallId);

    @Query("SELECT COUNT(s) FROM BusSeat s WHERE s.isBlocked = true AND s.isActive = true")
    Long countBlockedSeats();

    // --- Additional methods for Theater statistics ---

    /** Count total seats by theater ID */
    @Query("SELECT COUNT(s) FROM BusSeat s WHERE s.hall.theater.id = :theaterId AND s.isActive = true")
    Long countByTheaterIdAndIsActive(@Param("theaterId") String theaterId);

    /** ✅ FIXED: Count seats by theater admin user (not owner) */
    @Query("SELECT COUNT(s) FROM BusSeat s WHERE s.hall.theater.adminUserId = :adminUserId AND s.isActive = true")
    Long countByTheaterAdminUserIdAndIsActive(@Param("adminUserId") String adminUserId);

    /** Count seats by seat class in a hall */
    @Query("SELECT COUNT(s) FROM BusSeat s WHERE s.hall.id = :hallId AND s.seatClass = :seatClass AND s.isActive = true")
    ...
    @Query("SELECT COUNT(s) FROM BusSeat s WHERE s.hall.id = :hallId AND s.isBlocked = true AND s.isActive = true")
    Long countBlockedSeatsByHallId(@Param("hallId") String hallId);

    /** Get seat class distribution for a hall */
    @Query("SELECT s.seatClass, COUNT(s) FROM BusSeat s WHERE s.hall.id = :hallId AND s.isActive = true GROUP BY s.seatClass")
    List<Object[]> getSeatClassDistributionByHallId(@Param("hallId") String hallId);

    /** Find seats by row name in a hall */
    @Query("SELECT s FROM BusSeat s WHERE s.hall.id = :hallId AND s.rowName = :rowName AND s.isActive = true ORDER BY s.seatNumber ASC")
    ...
    @Query("SELECT DISTINCT s.rowName FROM BusSeat s WHERE s.hall.id = :hallId AND s.isActive = true ORDER BY s.rowName ASC")
    List<String> findDistinctRowNamesByHallId(@Param("hallId") String hallId);

    /** Find seats by theater ID (all halls) */
    @Query("SELECT s FROM BusSeat s WHERE s.hall.theater.id = :theaterId AND s.isActive = true ORDER BY s.hall.name, s.rowName, s.seatNumber")
    Page<Seat> findByTheaterIdAndIsActiveTrue(@Param("theaterId") String theaterId, Pageable pageable);

    /** Find blocked seats by hall */
    @Query("SELECT s FROM BusSeat s WHERE s.hall.id = :hallId AND s.isBlocked = true AND s.isActive = true ORDER BY s.rowName, s.seatNumber")
    List<Seat> findBlockedSeatsByHallId(@Param("hallId") String hallId);

    /** Find available (not blocked) seats by hall */
    @Query("SELECT s FROM BusSeat s WHERE s.hall.id = :hallId AND s.isBlocked = false AND s.isActive = true ORDER BY s.rowName, s.seatNumber")
    List<Seat> findAvailableSeatsByHallId(@Param("hallId") String hallId);

    /** Count seats by theater and seat class */
    @Query("SELECT COUNT(s) FROM BusSeat s WHERE s.hall.theater.id = :theaterId AND s.seatClass = :seatClass AND s.isActive = true")
    ...
    @Query("SELECT s.id FROM BusSeat s WHERE s.id IN :seatIds AND s.isActive = true")
    List<String> findExistingSeatIds(@Param("seatIds") List<String> seatIds);

    /** Find seats in a specific range */
    @Query("SELECT s FROM BusSeat s WHERE s.hall.id = :hallId AND s.rowName BETWEEN :startRow AND :endRow AND s.isActive = true ORDER BY s.rowName, s.seatNumber")
    ...
    @Query("SELECT COUNT(s) FROM BusSeat s WHERE s.isActive = true")
    ...
    @Query("SELECT s.hall.theater.id, s.hall.theater.name, COUNT(s), SUM(CASE WHEN s.isBlocked = true THEN 1 ELSE 0 END) " +
            "FROM BusSeat s WHERE s.isActive = true GROUP BY s.hall.theater.id, s.hall.theater.name")
    List<Object[]> getSeatStatisticsByTheater();

    /** Soft delete (deactivate) seats by hall ID */
    @Query("UPDATE BusSeat s SET s.isActive = false WHERE s.hall.id = :hallId")
